    ...
    <div className="relative overflow-hidden rounded-3xl border border-zinc-800">
      ...
      <div className="pointer-events-none absolute inset-0 flex items-end">
        <div className="pointer-events-auto mx-auto w-full max-w-6xl px-6 pb-8">
          ...
          {subtitle && <p className="mt-2 max-w-2xl text-zinc-200">{subtitle}</p>}

          <div className="mt-4 flex gap-3">
            {buttonPrimary && (
              <button className="rounded-3xl bg-white px-4 py-2 font-semibold text-black transition active:scale-95 hover:bg-zinc-200">
                {buttonPrimary}
              </button>
            )}
            {buttonSecondary && (
              <button className="rounded-3xl border border-zinc-700 px-4 py-2 font-semibold text-white transition active:scale-95 hover:text-orange-500">
                {buttonSecondary}
              </button>
            )}
          </div>
